...
import { Clock3, Download, Eye, IndianRupee, MessageCircleQuestion, Printer, Search, Send, Wrench } from 'lucide-react';
...
// POCreation.tsx / FinanceAdminOpsIndent.tsx — all three pages read the same
// backend record, this one just scopes to SPR (Work Order) rows.
const mapAdminPurchaseOrderToRecord = (order: any): PoApprovalRecord => {
  ...
  return {
    id: orderNumber || `wo-approval-${Date.now()}`,
    ...
    revisionNo: Number(quote?.amendmentNo ?? 0) || 0,
    questions: questions.map((q) => ({
      id: String(q?.id ?? `wo-question-${Date.now()}`),
...
const fetchPoApprovalsFromBackend = async (): Promise<PoApprovalRecord[]> => {
  try {
    ...
    toast.error(error?.message || 'Failed to load WO approvals');
    return [];
  }
};
...
export default function WOCreation() {
  ...
  const downloadRegisterPdf = async () => {
    ...
    try {
      ...
      toast.success('WO Register PDF downloaded');
    } catch (error: any) {
      toast.error(`Failed to generate WO Register PDF: ${safe(error?.message) || 'Unknown error'}`);
    } finally {
  ...
  const printRegisterPdf = async () => {
    ...
    try {
      ...
      toast.error(`Failed to print WO Register PDF: ${safe(error?.message) || 'Unknown error'}`);
    } finally {
    ...
    <div className="min-h-screen space-y-6 bg-slate-50/70 p-4 sm:p-6 lg:p-8">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#0D3A35]">Procurement · Work Order</p>
            <h1 className="mt-1 text-3xl font-semibold tracking-tight text-slate-950">WO Creation</h1>
            <p className="mt-1 text-sm text-slate-500">Create work orders from NFA-approved service comparative statements.</p>
          </div>
        </div>
        <div className="flex self-start gap-2 lg:self-auto">
          ...
          >
            <Printer className="h-4 w-4" /> {pdfPrinting ? 'Preparing Print...' : 'Print WO Register'}
          </Button>
          <Button
            type="button"
      ...
      <section className="grid overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:grid-cols-2">
        {[
          ...
          { label: "No. Of WO's Created", value: Object.keys(createdOrders).length, icon: Wrench },
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div><h2 className="font-bold text-slate-900">Approved Statement Register</h2><p className="mt-0.5 text-xs text-slate-500">Only NFA-approved records are eligible for WO creation.</p></div>
          <label className="relative block w-full lg:w-[380px]">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search comparison, SR, vendor or service" className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm outline-none focus:border-[#0D3A35] focus:bg-white" />
          </label>
        </div>
          ...
          <table className="w-full table-fixed border-collapse">
            <thead className="bg-[#0D3A35] text-white">
              <tr className="text-center text-[10px] uppercase tracking-[0.05em]">
                ...
                <th className="w-[10%] px-2 py-2.5">SR Number</th>
                <th className="w-[11%] px-2 py-2.5">WO Number</th>
                <th className="w-[12%] px-2 py-2.5">Approved Vendor</th>
                <th className="w-[14%] px-2 py-2.5">Service Details</th>
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-1.5">
                        {approvalRecord?.status === 'approved' ? (
                          ...
                          <Button
                            ...
                            className="h-8 bg-[#0D3A35] px-2 text-xs text-white hover:bg-[#092e2a]"
                          >
                            {orderNo ? 'View / Edit WO' : 'Create WO'}
                          </Button>
                        )}
      ...
      <MakeWorkOrderPopup
        ...
        onConfirm={async ({ indentId, poNo }) => {
          ...
          const wasAlreadyApproved = poApprovalRecords.some(
            (record) => record.prNumber === indentId && record.status === 'approved',
          );

          // The WO was already saved to the backend by MakeWorkOrderPopup's own
          // onConfirm handler — director_approval is set to "pending"
          // automatically on every create/update, so just re-fetch to pick it up.
          const refreshed = await refreshPoState();
          ...
          const approvalRecord = refreshed.find((record) => record.poNumber === poNo) || {
            id: poNo,
            poNumber: poNo,
            orderType: 'SPR',
            prNumber: indentId,
          ...
          toast.success(wasAlreadyApproved ? `Work Order ${poNo} revised` : `Work Order ${poNo} created`);
        }}
      />
      ...
      <Dialog open={Boolean(questionReplyRecord)} onOpenChange={(open) => { if (!open) { setQuestionReplyRecord(null); setReplyDrafts({}); } }}>
        <DialogContent className="max-h-[88vh] max-w-2xl overflow-hidden rounded-2xl border-0 p-0 shadow-2xl">
          <DialogHeader className="bg-[#0D3A35] px-6 py-5 text-left text-white">
            ...
            <p className="pl-[52px] text-sm text-white/70">WO {questionReplyRecord?.poNumber || '—'} · Reply to Admin Ops Finance</p>
          </DialogHeader>
                  ...
                  <div className="space-y-3 px-4 py-4">
                    ...
                    <div className="flex justify-end">
                      <Button
                        ...
                        disabled={!safe(replyDrafts[question.id]) || replyingQuestionId === question.id}
                        onClick={async () => {
                          if (!questionReplyRecord || !safe(replyDrafts[question.id])) return;
                          const repliedBy = readUserProfile().name?.trim() || 'WO Creator';
      ...
      <Dialog open={Boolean(approvalPrompt)} onOpenChange={(open) => { if (!open) setApprovalPrompt(null); }}>
        <DialogContent className="max-w-md overflow-hidden rounded-2xl border-0 p-0 shadow-2xl">
          <DialogHeader className="bg-[#0D3A35] px-6 py-5 text-left text-white">
            ...
            <DialogTitle className="text-xl font-bold text-white">Sent for Approval</DialogTitle>
            <DialogDescription className="text-sm text-white/70">
              Work Order {approvalPrompt?.poNumber || '—'} has been {approvalPrompt?.revisionNo ? 'revised' : 'created'} successfully.
            </DialogDescription>
          </DialogHeader>
          <div className="px-6 py-5">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-sm text-slate-500">The {approvalPrompt?.revisionNo ? 'revised WO' : 'WO'} has already been routed to Finance Admin Ops under WO Approvals — no further action needed here.</p>
            </div>
          </div>
