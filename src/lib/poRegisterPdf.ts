...
export type PoRegisterPdfOptions = {
  generatedBy?: string;
  // Lets the same register renderer produce a Work Order register instead of
  // a Purchase Order one — defaults preserve the original PO wording exactly.
  orderLabel?: string;
  orderCode?: string;
};
...
const drawFirstPageHeader = (doc: jsPDF, logoDataUrl: string, registerTitle: string) => {
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(10);
  doc.text(registerTitle, 105, 65, { align: 'center' });
};
...
export const buildPoRegisterPdfDoc = async (
  ...
  const orderLabel = options.orderLabel?.trim() || 'Purchase Order';
  const orderCode = options.orderCode?.trim() || 'PO';

  let logoDataUrl = '';
  try { logoDataUrl = await imageAsDataUrl(logo3f); } catch { logoDataUrl = ''; }

  drawFirstPageHeader(doc, logoDataUrl, `${orderLabel.toUpperCase()} REGISTER`);
  ...
  drawSectionBand(doc, 84, `${orderLabel}s`);
  ...
  const body: RowInput[] = rows.length
    ...
    : [[{ content: `No ${orderLabel.toLowerCase()} records found.`, colSpan: 10, styles: { halign: 'center', textColor: MUTED_SLATE } }]];

  autoTable(doc, {
    startY: 91,
    head: [['S.No.', 'Comparative No.', 'PR Number', `${orderCode} Number`, 'Approved Vendor', 'Item Details', 'UOM', 'Qty.', 'Status', `${orderCode} Value`]],
    body,
    theme: 'grid',
    rowPageBreak: 'avoid',
  ...
  drawSummary(doc, summaryY, [
    ...
    { label: `${orderCode}s Created`, value: String(createdRows.length) },
    { label: 'Pending Creation', value: String(rows.length - createdRows.length) },
    { label: `Total ${orderCode} Value`, value: `Rs. ${formatNumber(totalPoValue)}` },
  ]);
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    if (page > 1) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(20, 45, 76);
      doc.text(`${COMPANY_NAME} - ${orderLabel.toUpperCase()} REGISTER`, 12, 17);
      doc.setDrawColor(...PROJECT_GREEN);
    ...
    doc.setFontSize(5.8);
    doc.setTextColor(...MUTED_SLATE);
    doc.text(`System-generated ${orderLabel} Register`, 12, 284);
...
export const downloadPoRegisterAsPdf = async (
  ...
  const orderCode = options.orderCode?.trim() || 'PO';
  doc.save(`${orderCode}_Register_${new Date().toISOString().slice(0, 10)}.pdf`);
};
...
export const printPoRegisterAsPdf = async (
  ...
  const orderLabel = options.orderLabel?.trim() || 'Purchase Order';
  try {
    printWindow.document.title = `Generating ${orderLabel} Register...`;
    const doc = await buildPoRegisterPdfDoc(rows, options);
    doc.autoPrint();
