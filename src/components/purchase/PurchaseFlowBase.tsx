...
// Trailing sequence number of an order number like "SBRPL/PO/BIO-CG/26-27/009"
// or "SBRPL/WO/BIO-CG/26-27/007" — the part used to sort the register newest-first.
const parseOrderNumberSeq = (orderNumber: unknown): number | null => {
  const parts = safeTrim(orderNumber).split('/');
...
type PurchaseFlowBaseProps = {
  orderTypeFilter?: 'PR' | 'SPR';
  title?: string;
};

export default function PurchaseFlowBase({ orderTypeFilter, title }: PurchaseFlowBaseProps = {}) {
  ...
  const rows = useMemo(() => {
    ...
    // Newest PO/WO number first (e.g. .../009 → .../008 → .../007 → ...) — falls
    // back to timestamp for rows without a parseable trailing sequence number
    // (e.g. drafts) so they don't land arbitrarily among the numbered ones.
    copy.sort((a, b) => {
      const seqA = parseOrderNumberSeq((a as any)?.order_number);
      const seqB = parseOrderNumberSeq((b as any)?.order_number);
      if (seqA !== null && seqB !== null && seqA !== seqB) return seqB - seqA;
      if (seqA !== null && seqB === null) return -1;
      if (seqA === null && seqB !== null) return 1;
      return safeTrim((b as any)?.timestamp).localeCompare(safeTrim((a as any)?.timestamp));
    });
    return copy;
