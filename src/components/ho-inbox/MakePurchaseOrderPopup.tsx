    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-5">
      ...
      <section style={{ order: 7 }} className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
        ...
        <div className="flex flex-wrap items-end gap-2 border-b border-slate-300 bg-[#edf4f2] px-4 py-3">
          ...
          <div><label className="block text-[9px] font-bold uppercase text-[#0D3A35]">Rows</label><Input type="number" onWheel={(e) => e.currentTarget.blur()} min={1} max={20} value={preTableRows} onChange={(event) => setPreTableRows(Number(event.target.value))} className="mt-1 h-8 w-16 bg-white text-center text-xs" /></div>
          <div><label className="block text-[9px] font-bold uppercase text-[#0D3A35]">Columns</label><Input type="number" onWheel={(e) => e.currentTarget.blur()} min={1} max={10} value={preTableColumns} onChange={(event) => setPreTableColumns(Number(event.target.value))} className="mt-1 h-8 w-16 bg-white text-center text-xs" /></div>
          ...
          <div className="grid w-full grid-cols-[minmax(0,0.85fr)_minmax(0,1.4fr)_minmax(0,1fr)] items-stretch gap-1.5">
            ...
            <div className="flex min-h-[64px] min-w-0 flex-col justify-between border-r border-t border-slate-300 px-1 pt-2">
              ...
              <div className="flex flex-wrap items-center justify-center gap-1">
                ...
                <Input aria-label="Table rows" title="Table rows" type="number" onWheel={(e) => e.currentTarget.blur()} min={1} max={20} value={annexureTableRows} onChange={(event) => setAnnexureTableRows(Number(event.target.value))} className="h-8 w-10 rounded-md bg-white px-1 text-center text-[10px]" /><span className="text-[10px]">×</span><Input aria-label="Table columns" title="Table columns" type="number" onWheel={(e) => e.currentTarget.blur()} min={1} max={10} value={annexureTableColumns} onChange={(event) => setAnnexureTableColumns(Number(event.target.value))} className="h-8 w-10 rounded-md bg-white px-1 text-center text-[10px]" />
