...
function StatusChip({ status }: { status: Status }) {
  return <span className={cn("inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold", status === "Active" ? "bg-emerald-50 text-emerald-700" : status === "Suspended" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500")}>{status}</span>;
}
...
const nextCostCentreCode = (department: string, centres: CostCentre[]) => {
  const suffix = (department || "GEN").replace(/[^a-z0-9]/gi, "").slice(0, 3).toUpperCase() || "GEN";
...
function CostCentreDrawer({ mode, item, centres, departments, onClose, onSave }: { mode: DrawerMode; item?: CostCentre; centres: CostCentre[]; departments: MasterOption[]; onClose: () => void; onSave: (item: CostCentre) => void }) {
  const blank: CostCentre = { id: `cc-${Date.now()}`, code: nextCostCentreCode("", centres), name: "", shortName: "", parent: "", type: "Department", department: "", head: "", secondary: "", entity: "SAI BIORESOURCES PRIVATE LIMITED", description: "", budgetApplicable: false, budgetControl: "Informational", annualBudget: 0, directPosting: true, prrPosting: true, purchasePosting: true, payrollPosting: false, ledgerMapping: "", effectiveFrom: new Date().toISOString().slice(0, 10), effectiveTo: "", status: "Active" };
  const [form, setForm] = useState<CostCentre>(item || blank);
  ...
  if (mode === "view" && item) return <Drawer centered eyebrow="Cost Centre Details" title={item.name} onClose={onClose}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[["Cost Centre Code", item.code], ["Cost Centre Name", item.name], ["Parent Cost Centre", item.parent], ["Department", item.department], ["Cost Centre Head", item.head], ["Budget Applicable", item.budgetApplicable ? "Yes" : "No"], ["Status", item.status], ["Description", item.description]].map(([label, value]) => <div key={String(label)} className={cn("rounded-xl border border-slate-200 bg-white p-4", label === "Description" && "sm:col-span-2 lg:col-span-3")}><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">{label}</p><p className="mt-2 text-sm font-bold text-slate-800">{String(value || "—")}</p></div>)}</div></Drawer>;
  const footer = <div className="flex justify-end gap-3"><button onClick={onClose} className="h-10 rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-600">Cancel</button><button onClick={save} className="h-10 rounded-xl bg-[#0d5c4d] px-5 text-sm font-bold text-white">Save Cost Centre</button></div>;
  return <Drawer centered eyebrow="Cost Accounting · Cost Centre" title={mode === "add" ? "Add Cost Centre" : "Edit Cost Centre"} onClose={onClose} footer={footer}>
    <FormSection title="Cost Centre Details">
      <label className={labelClass}>Cost Centre Code<input readOnly className={cn(fieldClass, "bg-slate-50 font-mono text-[#0d5c4d]")} value={form.code} /><span className="text-[10px] font-medium text-slate-400">Auto-generated from the selected department</span></label>
      ...
      <label className={labelClass}>Department<select className={fieldClass} value={form.department} onChange={(e) => { const department = e.target.value; setForm((current) => ({ ...current, department, head: current.head || (department ? `${department} Head` : ""), code: mode === "add" ? nextCostCentreCode(department, centres) : current.code })); }}><option value="">{departments.length ? "Select department" : "Configure departments in Organisation"}</option>{departments.map((department) => <option key={department.id} value={department.label}>{department.label}{department.id !== department.label ? ` · ${department.id}` : ""}</option>)}</select>{errors.department && <span className="text-[11px] text-red-600">{errors.department}</span>}</label>
      <label className={labelClass}>Cost Centre Head<select className={fieldClass} value={form.head} onChange={(e) => update("head", e.target.value)}><option value="">Select cost centre head</option>{headOptions.map((head) => <option key={head}>{head}</option>)}</select></label>
      <label className={labelClass}>Status<select className={fieldClass} value={form.status === "Suspended" ? "Inactive" : form.status} onChange={(e) => update("status", e.target.value as Status)}><option>Active</option><option>Inactive</option></select></label>
      <label className={cn(labelClass, "sm:col-span-2")}>Description<textarea className="min-h-24 w-full rounded-xl border border-slate-200 p-3 text-sm" value={form.description} onChange={(e) => update("description", e.target.value)} placeholder="e.g. Farm-related operational expenses" /></label>
      <div className="sm:col-span-2"><Switch checked={form.budgetApplicable} onChange={(value) => update("budgetApplicable", value)} label="Budget Applicable" /></div>
    </FormSection>
  </Drawer>;
}
...
export default function CostAccountingSetup({ mode, projects = [], departments = [], legalEntity = "SAI BIORESOURCES PRIVATE LIMITED" }: { mode: CostAccountingMode; projects?: Array<{ id?: string; code?: string; name: string; status?: string }>; departments?: Array<{ id?: string; code?: string; name: string; status?: string }>; legalEntity?: string }) {
  ...
  const [centres, setCentres] = useState<CostCentre[]>(() => loadUserRecords<CostCentre>(CENTRE_STORAGE, SEEDED_CENTRE_IDS));
  ...
  const filtered = useMemo(() => rows.filter((row) => {
    ...
    const scope = isCentre ? (row as CostCentre).department : (row as Attribution).project;
  ...
  const exportCsv = () => { const headers = isCentre ? ["Code", "Name", "Parent", "Type", "Department", "Head", "Status"] : ["Code", "Name", "Level", "Parent", "Linked Master", "Allocation", "Status"]; const data = rows.map((row) => isCentre ? [(row as CostCentre).code, row.name, (row as CostCentre).parent, (row as CostCentre).type, (row as CostCentre).department, (row as CostCentre).head, row.status] : [(row as Attribution).code, row.name, (row as Attribution).level, (row as Attribution).parent, (row as Attribution).linkedMaster, (row as Attribution).allocation, row.status]); const blob = new Blob([[headers, ...data].map((line) => line.map((cell) => `"${cell}"`).join(",")).join("\n")], { type: "text/csv" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${mode.toLowerCase().replaceAll(" ", "-")}.csv`; anchor.click(); URL.revokeObjectURL(url); };
  const columns = isCentre ? ["Parent", "Type", "Department", "Head", "Budget Control"] : ["Level", "Parent", "Linked Master", "Allocation"];

  return <div className="space-y-5">
    <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 xl:flex-row xl:items-start xl:justify-between"><div className="grid flex-1 gap-2 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6"><label className="relative sm:col-span-2"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input className={cn(fieldClass, "pl-9")} value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search code, name, project, cluster or land parcel" /></label><select className={fieldClass} value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}><option>All Types</option>{(isCentre ? ["Corporate", "Department", "Functional", "Project", "Operational", "Support", "Temporary"] : ["Corporate", "Project", "Cluster", "Zone", "Block", "Land"]).map((value) => <option key={value}>{value}</option>)}</select><select className={fieldClass} value={scopeFilter} onChange={(e) => setScopeFilter(e.target.value)}><option value="All">{isCentre ? "All Departments" : "All Projects"}</option>{Array.from(new Set(rows.map((row) => isCentre ? (row as CostCentre).department : (row as Attribution).project).filter(Boolean))).map((value) => <option key={value}>{value}</option>)}</select><select className={fieldClass} value={parentFilter} onChange={(e) => setParentFilter(e.target.value)}><option value="All">{isCentre ? "All Parents" : "All Clusters"}</option>{Array.from(new Set(rows.map((row) => isCentre ? (row as CostCentre).parent : (row as Attribution).cluster).filter(Boolean))).map((value) => <option key={value}>{value}</option>)}</select>{!isCentre && <select className={fieldClass} value={masterFilter} onChange={(e) => setMasterFilter(e.target.value)}><option value="All">All Linked Masters</option>{Array.from(new Set(attributions.map((row) => row.linkedMaster))).map((value) => <option key={value}>{value}</option>)}</select>}<select className={fieldClass} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option>All Statuses</option><option>Active</option><option>Inactive</option><option>Suspended</option></select></div><div className="flex flex-wrap gap-2"><button onClick={() => { setQuery(""); setTypeFilter("All Types"); setStatusFilter("All Statuses"); setScopeFilter("All"); setParentFilter("All"); setMasterFilter("All"); }} className="rounded-xl border border-slate-200 p-2.5 text-slate-500" title="Refresh"><RefreshCw className="h-4 w-4" /></button><button onClick={exportCsv} className="rounded-xl border border-slate-200 p-2.5 text-slate-500" title="Export"><Download className="h-4 w-4" /></button>{!isCentre && <button onClick={() => setPreview(true)} className="h-10 rounded-xl border border-[#b8d6ce] px-4 text-sm font-bold text-[#0d5c4d]">Preview PRR</button>}<button onClick={() => setDrawer({ mode: "add" })} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#0d5c4d] px-4 text-sm font-bold text-white"><Plus className="h-4 w-4" />{isCentre ? "Add Cost Centre" : "Add Attribution"}</button></div></div>
    ...
    {drawer && isCentre && <CostCentreDrawer mode={drawer.mode} item={drawer.item as CostCentre | undefined} centres={centres} departments={departmentOptions} onClose={() => setDrawer(null)} onSave={saveCentre} />}
